...
class PengujiGudepController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        if ($request->ajax()) {
            ...
            $data = AccountPengujiGudep::leftjoin('pengurus_gudeps', 'account_penguji_gudeps.gudep_id', '=', 'pengurus_gudeps.id')->select('account_penguji_gudeps.id', 'pengurus_gudeps.name', 'account_penguji_gudeps.email', 'account_penguji_gudeps.created_at')->latest()->get();
            ...
            return datatables::of($data)
                    ...
                    ->filter(function ($instance) use ($request) {
                        if (!empty($request->get('pengurus_id'))) {
                            $instance->collection = $instance->collection->filter(function ($row) use ($request) {
                                return Str::contains($row['pengurus_id'], $request->get('pengurus_id')) ? true : false;
                            });
                        }

                        if (!empty($request->get('search'))) {
                            $instance->collection = $instance->collection->filter(function ($row) use ($request) {
                                if (Str::contains(Str::lower($row['pengurus_id']), Str::lower($request->get('search')))){
                                    return true;
                                }
                                return false;
                            });
